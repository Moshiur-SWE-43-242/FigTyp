...
function getGeminiClient(): GoogleGenAI {
  if (!aiInstance) {
    ...
    if (!apiKey) {
      throw new Error('GEMINI_API_KEY is not defined in the workspace secrets or environment variables.');
    }
...
async function startServer() {
  ...
  // AI - API Neural Coach evaluate route
  app.post('/api/ai/coach', async (req, res) => {
    const { currentMode, errorsLogged } = req.body;

    // Fetch user historical performance to customize coaching
    let userContextStr = '';
    if (req.user) {
      const attempts = db.getAttempts().filter(a => a.userId === req.user?.id);
      if (attempts.length > 0) {
        const avgWpm = Math.round(attempts.reduce((acc, a) => acc + a.wpm, 0) / attempts.length);
        const bestWpm = Math.max(...attempts.map(a => a.wpm));
        const avgAcc = (attempts.reduce((acc, a) => acc + a.accuracy, 0) / attempts.length).toFixed(1);
        userContextStr = `The student has general historical averages of ${avgWpm} WPM (Peak: ${bestWpm} WPM) and ${avgAcc}% accuracy.`;
      }
    }

    const currentModeDescriptor = currentMode || 'quote speedrun';
    const errorFormatted = JSON.stringify(errorsLogged || {});

    const prompt = `You are the official FigType AI Neural Coach. FigType is an enterprise-grade AI-powered typing ecosystem founded in 2026 by Software Engineering Student Md Moshiur Rahaman Riat under the auspices of MiraCore Logix and M-Square Devs Group. 
Analyze the current student's performance metrics:
- Typing Mode: ${currentModeDescriptor}
- Incorrect fingers / key error tracking map: ${errorFormatted}
${userContextStr}

Write an elegant, engaging review:
1. "NEURAL SPEED PROFILE": Create a short technical mock speed profile rating (e.g. "Staccato Flow", "Linear Pulse", "Asymmetrical Rhythm") based on their accuracy and errors.
2. "WEAK KEY DRILLS": Formulate exactly 3 customized practice lines (word sequences targeting their specific error keys). Make sure they are engaging to type!
3. "POSTURE & DEXTERITY TIPS": Give 2 specialized physical instructions (finger mechanics or keyboard heights).
4. "MIRACORE DAILY FOCUS": Output an encouraging motivational slogan or daily focus mantra for their future tests.

Write the output entirely in crisp, beautiful Markdown with neon slate aesthetics (e.g. using bullet points, short clear tables or quotes). Maintain a highly professional, encouraging, and tech-forward coaching voice. Do not write excessively verbose text. Keep it focused and visual.`;
    ...
    try {
      ...
      try {
        ...
        console.warn('Primary model gemini-3.5-flash failed or was throttled, seeking fallback gemini-3.1-flash-lite...', firstErr.message || firstErr);
        // Fallback model
        response = await ai.models.generateContent({
      ...
      res.json({
        success: true,
        coachRemark: response.text || 'Error parsing AI response structure.'
      });
    } catch (err: any) {
      console.error('Gemini API call skipped or failed:', err.message);
      // Fallback elegant dashboard evaluation if Gemini API is not yet active or credentialed properly!
      res.json({
        success: false,
        message: err.message,
        coachRemark: `### Neural Coach Analytics (Interactive Analyzer Mode)

We noticed your **GEMINI_API_KEY** is not configured yet. Here is an automatic, offline analytical response generated by the local **MiraCore Logix Pattern System**:

#### 📊 Local Analytics Breakdowns:
* **Neural Profile:** **Targeted Staccato Rhythm**. Slow starts with rapid terminal bursts indicate high-focus focus but a lack of initial rhythmic confidence.
* **Weak Key Analysis:** Your mistyped key signatures point to **${Object.keys(errorsLogged || {}).length > 0 ? Object.keys(errorsLogged || {}).join(', ').toUpperCase() : 'stabilized outer-finger'} key positions**. Under-extending the ring and pinky fingers during fast sequences accounts for 85% of standard keystroke mistakes.

#### 🎯 Custom Warmup Exercises:
1. **Pinky Extension Sequence:** \`aza sza p;l aqz p;l pop\` (repeat 5 times focusing purely on rhythm)
2. **Double Strike Stability:** \`good school core wood group Riat\` (focus on fluid double letter transition jumps)

*Configure the **GEMINI_API_KEY** via the AI Studio Secrets panel anytime to unlock fully interactive live-streamed AI evaluations from the Gemini 3.5 model!*`
      });
    }
  });

  // AI - Custom Q&A ask route (for message & voice questions)
  app.post('/api/ai/ask', async (req, res) => {
    const { message, testMode, errorsLogged } = req.body;
    if (!message) {
      return res.status(400).json({ error: "Message is required." });
    }

    // Prepare user historical context
    let userContextStr = '';
    if (req.user) {
      const attempts = db.getAttempts().filter(a => a.userId === req.user?.id);
      if (attempts.length > 0) {
        const avgWpm = Math.round(attempts.reduce((acc, a) => acc + a.wpm, 0) / attempts.length);
        const avgAcc = (attempts.reduce((acc, a) => acc + a.accuracy, 0) / attempts.length).toFixed(1);
        userContextStr = `The student's historical typing stats are: ${avgWpm} WPM average speed with ${avgAcc}% accuracy.`;
      }
    }

    const currentModeDescriptor = testMode || 'standard session';
    const errorKeys = Object.keys(errorsLogged || {}).join(', ').toUpperCase() || 'stabilized keys';

    const systemInstructions = `You are the FigType AI Coach, an enterprise-grade interactive typing coach designed by Md Moshiur Rahaman Riat under MiraCore Logix.
    The student is typing in mode "${currentModeDescriptor}" and has issues with keys: "${errorKeys}".
    ${userContextStr}
    
    The student is asking you a direct question (either submitted via keyboard typing or spoken via voice transcription):
    "${message}"
    
    Answer their question in a highly encouraging, professional, and clear manner. If they ask about typing tips, speed hacks, posture, or muscle memory, give specific and practical advice.
    Respond in beautiful, well-formatted Markdown with clean section headers. Use bullet points where appropriate. Keep the output beautifully concise and visual.`;
    ...
    try {
      ...
      try {
        ...
        console.warn('Primary model gemini-3.5-flash failed during Q&A, fallback to gemini-3.1-flash-lite...', firstErr.message || firstErr);
        response = await ai.models.generateContent({
          model: 'gemini-3.1-flash-lite',
          contents: systemInstructions
        });
      ...
      res.json({
        success: true,
        answer: response.text || 'Error generating response.'
      });
    } catch (err: any) {
      console.error('Gemini API call custom question failed:', err.message);
      // Fallback response:
      res.json({
        success: false,
        message: err.message,
        answer: `### 🤖 AI Coach Handshake Response
        
I received your query: *"${message}"*

Currently, the server is running offline or the Gemini API key needs activation. Here is a helpful tip inspired by **MiraCore Logix Help Center**:
- To improve typing on keys like **${errorKeys}**, match your finger positioning strictly to the virtual keyboard. Practice slow, consistent strides before speeding up. Keep your shoulders down and wrists elevated!`
      });
    }
  });
  ...
  // API - AI Help Center chatbot
  app.post('/api/ai/helpcenter', async (req, res) => {
    const { question } = req.body;
    if (!question) {
      return res.status(400).json({ error: 'Please enter a valid question.' });
    }

    const brandContextPragmatic = `You are the official FigType AI Support Assistant, representing M-Square Devs Group and SWE student Md Moshiur Rahaman Riat. You are an expert on typing techniques, keyboard economics, finger muscle memory, and general ergonomics (Colemak, Dvorak, QWERTY layouts). 
A student asks: "${question}"
Answer their question clearly, professionally, and concisely in clean Markdown, with an esports-inspired, friendly, encouraging tone. Keeping formatting clean and visual.`;
    ...
    try {
      ...
      try {
        response = await ai.models.generateContent({
          model: 'gemini-3.5-flash',
          contents: brandContextPragmatic
        });
      } catch (e1) {
        console.warn('Primary model failed for help center, switching to fallback...', e1);
        response = await ai.models.generateContent({
          model: 'gemini-3.1-flash-lite',
          contents: brandContextPragmatic
        });
      ...
      res.json({
        success: true,
        answer: response.text || 'Error synthesising support answer.'
      });
    } catch (err: any) {
      console.error('Error in help center API:', err);
      res.json({
        success: false,
        answer: `### FigType Support Assistant (Local Knowledge Mode)
        
We are currently operating in local offline pattern mode. 

To achieve higher typing speed (**WPM**), focus on:
1. **Home Row Anchor:** Keep your index fingers anchored on **F** and **J** tactile ridges.
2. **Wrist Elevation:** Elevate wrists slightly so fingers have vertical hammer-angles.
3. **Accuracy first:** Speed is a byproduct of precise muscle memory. Slow down to 98% accuracy and watch your speeds climb safely!`
      });
    }
  });
