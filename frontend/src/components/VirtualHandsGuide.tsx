...
export default function VirtualHandsGuide({ targetKey, className = '' }: Props) {
  ...
  return (
    <div className={`p-4 rounded-2xl bg-slate-950/60 border border-slate-800/80 font-mono text-xs select-none ${className}`}>
      
      {/* Target indicator banner */}
      <div className="flex items-center justify-between mb-3 px-2">
        <div className="flex items-center gap-2">
          <span className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold">Touch Typing Guidance:</span>
          <span
            className="px-2 py-0.5 rounded text-[11px] font-bold shadow-sm transition-all duration-150"
            style={{ backgroundColor: `${fingerInfo.color}20`, color: fingerInfo.color, border: `1px solid ${fingerInfo.color}50` }}
          >
            {fingerInfo.label} &bull; Key: [{targetKey === ' ' ? 'SPACE' : (targetKey || '—').toUpperCase()}]
          </span>
        </div>
        <span className="text-[9px] text-slate-500 uppercase">TypingClub &reg; Virtual Hands</span>
      </div>
      ...
      <div className="flex items-center justify-center gap-8 md:gap-16 py-2">
        ...
        <div className="flex flex-col items-center gap-2">
          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Left Hand</span>
          <div className="flex items-end gap-1.5 h-20 p-2 rounded-xl bg-slate-900/60 border border-slate-800/90 shadow-inner">
            
            {/* Left Pinky */}
            <div className="flex flex-col items-center gap-1">
              <span className="text-[8px] text-pink-400">A</span>
              <div
            ...
            {/* Left Ring */}
            <div className="flex flex-col items-center gap-1">
              <span className="text-[8px] text-purple-400">S</span>
              <div
            ...
            {/* Left Middle */}
            <div className="flex flex-col items-center gap-1">
              <span className="text-[8px] text-sky-400">D</span>
              <div
            ...
            {/* Left Index */}
            <div className="flex flex-col items-center gap-1">
              <span className="text-[8px] text-emerald-400">F</span>
              <div
                className={`w-4 h-14 rounded-t-full transition-all duration-200 ${
            ...
            {/* Left Thumb */}
            <div className="flex flex-col items-center gap-1">
              <span className="text-[8px] text-amber-400">_</span>
              <div
        ...
        {/* RIGHT HAND */}
        <div className="flex flex-col items-center gap-2">
          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Right Hand</span>
          <div className="flex items-end gap-1.5 h-20 p-2 rounded-xl bg-slate-900/60 border border-slate-800/90 shadow-inner">
            ...
            {/* Right Index */}
            <div className="flex flex-col items-center gap-1">
              <span className="text-[8px] text-emerald-400">J</span>
              <div
                className={`w-4 h-14 rounded-t-full transition-all duration-200 ${
            ...
            {/* Right Middle */}
            <div className="flex flex-col items-center gap-1">
              <span className="text-[8px] text-sky-400">K</span>
              <div
                className={`w-4 h-16 rounded-t-full transition-all duration-200 ${
            ...
            {/* Right Ring */}
            <div className="flex flex-col items-center gap-1">
              <span className="text-[8px] text-purple-400">L</span>
              <div
                className={`w-4 h-14 rounded-t-full transition-all duration-200 ${
